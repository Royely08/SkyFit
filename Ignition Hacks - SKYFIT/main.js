...
function confirmAdd() {
  ...
  let queryParams = [];
  if (selectedExerciseType) queryParams.push(`type=${selectedExerciseType}`);
  if (selectedLevel) queryParams.push(`difficulty=${selectedLevel.toLowerCase()}`);
  if (selectedMuscleGroup) queryParams.push(`muscle=${selectedMuscleGroup}`);
